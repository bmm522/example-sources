...
public class TokenGeneratorTest {
    ...
    void generatedAccessTokenTest() {
        ...
        assertThat(actual.getValue().startsWith("Bearer ")).isTrue();
        assertThat(actual.getClass()).isEqualTo(AccessToken.class);
    }

    @Test
    @DisplayName("RefreshToken 생성 전략 객체와 그에 맞는 메타데이터를 넣으면 prefix가 붙어서 RefreshToken 토큰이 발급된다")
    void generatedRefreshTokenTest() {
        TokenMetadata tokenMetadata = FixtureTokenMetadata.createTokenMetadataWithPrefix("RefreshToken ");
        when(refreshTokenCreationStrategy.execute(any())).thenReturn(FixtureToken.createRefreshToken());
